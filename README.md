...
- 路由器 LAN 地址：`192.168.0.1`
- DHCP 地址池：`192.168.0.5` 到 `192.168.0.254`
- 页面监听地址：`192.168.0.1:19999`
...
- 默认加载地址：`http://192.168.0.1:9999/ui/`。
- 支持保存面板端口和路径。
...
安装脚本会把路由器 LAN 地址设置为 `192.168.0.1`，并把 DHCP 地址池设置为 `192.168.0.5` 到 `192.168.0.254`。如果安装时正在通过旧地址 SSH 连接路由器，网络重启后连接会断开，这是正常现象，稍等片刻后改用新地址访问即可。

安装后会输出：

```text
ShellCrash 管理器已安装：http://192.168.0.1:19999/
```
